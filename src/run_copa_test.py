...
def write_prediction(preds, guids, task_name, prefix):
    preds_dict = {}
    ...
    torch.save(preds_dict, os.path.join("../outputs/", prefix+"_test_preds.p"))
    formatted_preds = convert_superglue_submission(preds_dict[task_name])
    write_jsonl(data=formatted_preds, path=os.path.join("../outputs/", prefix+"_COPA.jsonl"))

def convert_superglue_submission(pred_dict):
    lines = []
    for pred, guid in zip(list(pred_dict["preds"]), list(pred_dict["guids"])):
        lines.append({"idx": int(guid.split("-")[1]), "label": int(pred)})
    return lines
...
def load_and_cache_examples(args, processor, retrievers, relation_list, input_dir, evaluate=False, output_examples=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        for num in range(2):
            tmp_all_kgs_graphs, tmp_all_kgs_graphs_label_dict = load_graphs(
                cached_features_file + "_all_kgs_graphs_choice_{}.bin".format(num))
...
def main():
    ...
    model_g.add_arg("use_nell", bool, False, "nell")
